...
See [Noisy data-fidelity terms for diffusion posterior sampling](https://deepinv.org/auto_examples/sampling/demo_noisy_data_fidelity.html.md#sphx-glr-auto-examples-sampling-demo-noisy-data-fidelity-py) for a side-by-side explanation and implementation example.

#### Noisy data-fidelity terms

| **Class**                                                                                                                                | $-\nabla_{x_t} \log p_t(y|x_t)$                                                                                                                 | **Requirements and implementation**                                                                                                                                                              |
|------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [`deepinv.sampling.ALDDataFidelity`](https://deepinv.org/api/stubs/deepinv.sampling.ALDDataFidelity.html.md#deepinv.sampling.ALDDataFidelity)                       | $\lambda A^\top (A x_t - y) / (\sigma_y^2 + \gamma_t^2)$, with an annealing parameter $\gamma_t$ (default $\gamma_t=\sigma_t$)                  | No denoiser and no linear solve, so by far the cheapest; measures the mismatch at the noisy iterate $x_t$.                                                                                       |
| [`deepinv.sampling.ScoreSDEDataFidelity`](https://deepinv.org/api/stubs/deepinv.sampling.ScoreSDEDataFidelity.html.md#deepinv.sampling.ScoreSDEDataFidelity)             | $\lambda A^\top (A x_t - y_t) / (\sigma_y^2 + \gamma_t^2)$, with noised measurements $y_t = y + \sigma_t\epsilon$                               | Projection method assuming $\sigma_t > \sigma_y$; no denoiser.                                                                                                                                   |
| [`deepinv.sampling.ILVRDataFidelity`](https://deepinv.org/api/stubs/deepinv.sampling.ILVRDataFidelity.html.md#deepinv.sampling.ILVRDataFidelity)                     | $\lambda A^\dagger (A x_t - y_t) / (\sigma_y^2 + \gamma_t^2)$                                                                                   | As above, lifted by the pseudo-inverse $A^\dagger$ instead of $A^\top$.                                                                                                                          |
| [`deepinv.sampling.DPSDataFidelity`](https://deepinv.org/api/stubs/deepinv.sampling.DPSDataFidelity.html.md#deepinv.sampling.DPSDataFidelity)                       | $\lambda\nabla_{x_t} \| \forw{\denoiser{x_t}{\sigma_t}} - y \|$                                                                                 | Differentiable physics; backpropagates through the denoiser and forward operator.                                                                                                                |
| [`deepinv.sampling.PiGDMDataFidelity`](https://deepinv.org/api/stubs/deepinv.sampling.PiGDMDataFidelity.html.md#deepinv.sampling.PiGDMDataFidelity)                   | $\lambda J_D^\top A^\top (r_t^2 A A^\top + \sigma_y^2\mathrm{Id})^{-1} (A\denoiser{x_t}{\sigma_t} - y)$, with $r_t^2=\sigma_t^2/(1+\sigma_t^2)$ | Linear physics and Gaussian measurement noise ($\sigma_y$ is read from `physics.noise_model.sigma`); uses an exact spectral inverse for decomposable operators and conjugate gradient otherwise. |
