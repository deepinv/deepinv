...
def channel_norm(x):
    x = x - x.min(dim=-1, keepdim=True)[0].min(dim=-2, keepdim=True)[0]  # center the image around zero
    x = x / x.max(dim=-1, keepdim=True)[0].max(dim=-2, keepdim=True)[0]  # normalize the image to [0, 1]
    return x
