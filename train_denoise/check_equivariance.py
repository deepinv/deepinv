...
if model_test == 'unext':
    ...
    net = UNeXt(in_channels=3, out_channels=3, device=device, replk=1, pool_type='next').to(device)
    sigma = 0.1
...
# print(net)
print('The model has ', sum(p.numel() for p in net.parameters() if p.requires_grad), 'parameters')
