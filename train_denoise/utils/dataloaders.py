...
def get_div2k(train_patch_size, device="cpu"):
    ...
    div2k_dataset = DIV2K(
        root="/gpfsscratch/rech/nyd/commun/", download=False, transform=transform
    )
    psf_size = 31

    motion_generator = (
        MotionBlurGenerator((psf_size, psf_size), l=0.6, sigma=1, device=device)
        + SigmaGenerator(device=device)
        + SigmaGenerator(sigma_min=0.01, sigma_max=0.5, device=device)
    )

    diffraction_generator = DiffractionBlurGenerator(
        psf_size=(psf_size, psf_size), num_channels=1, device=device
    ) + SigmaGenerator(sigma_min=0.01, sigma_max=0.5, device=device)
    gaussian_blur_generator = GaussianBlurGenerator(
        psf_size=(psf_size, psf_size), num_channels=1, device=device
    ) + SigmaGenerator(sigma_min=0.01, sigma_max=0.5, device=device)

    generator = GeneratorMixture(
        [motion_generator, diffraction_generator, gaussian_blur_generator],
        [1 / 3.0, 1 / 3.0, 1 / 3.0],
    )
...
def get_inpainting_physics(train_patch_size, device="cpu"):
    ...
    generator = InpaintingMaskGenerator(
        (train_patch_size, train_patch_size),
        block_size_ratio=0.2,
        num_blocks=10,
        device=device,
    ) + SigmaGenerator(sigma_min=0.01, sigma_max=0.5, device=device)
...
def get_SR_physics(train_patch_size, device="cpu", factor=2):
    ...
    generator = SigmaGenerator(sigma_min=0.0, sigma_max=0.05, device=device)

    return generator, physics


def get_drunet_dataset(
    train_patch_size,
    device="cpu",
    train=True,
    train_pth="/pth/to/train",
    val_pth="/pth/to/val",
    sigma_min=0.01,
    sigma_max=0.5,
):
    ...
    if train:
        dataset = torchvision.datasets.ImageFolder(root=train_pth, transform=transform)
    else:
        dataset = torchvision.datasets.ImageFolder(root=val_pth, transform=transform)

    physics = dinv.physics.DecomposablePhysics(
        device=device, noise_model=GaussianNoise(sigma=0.1)
    )
...
def get_BSD68(train_patch_size, device="cpu"):
    ...
    # Define the transform pipeline
    transform = transforms.Compose(
        [transforms.ToTensor(), transforms.CenterCrop(target_size)]
    )

    bsd68_dataset = torchvision.datasets.ImageFolder(
        root="/gpfsscratch/rech/fwf/commun/CBSD68/", transform=transform
    )
    psf_size = 31
...
def get_fastMRI(train_patch_size, device="cpu", train=True):
    ...
    fastmri_dataset = dinv.datasets.FastMRISliceDataset(
        root="/gpfsscratch/rech/nyd/commun/fastMRI/knee_singlecoil/singlecoil_"
        + str(training_str)
        + "/",
