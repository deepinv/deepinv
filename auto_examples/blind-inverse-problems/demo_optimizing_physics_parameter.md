...
[`deepinv.physics.Physics`](https://deepinv.org/api/stubs/deepinv.physics.Physics.md#deepinv.physics.Physics) together with automatic differentiation to estimate unknown parameters of your forward operator.
...
where $N$ is the noise model, $\forw{\cdot, \theta}$ is the forward operator, and the goal is to learn the parameter $\theta$ (e.g., the filter in [`deepinv.physics.Blur`](https://deepinv.org/api/stubs/deepinv.physics.Blur.md#deepinv.physics.Blur)).
...
In this first example, we use the convolution operator, defined in the [`deepinv.physics.Blur`](https://deepinv.org/api/stubs/deepinv.physics.Blur.md#deepinv.physics.Blur) class.
