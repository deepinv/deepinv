...
#### NOTE
Linear forward operators that have a [`closed-form singular value decomposition`](https://deepinv.org/api/stubs/deepinv.physics.DecomposablePhysics.md#deepinv.physics.DecomposablePhysics) benefit from a [`closed-form formula`](https://deepinv.org/api/stubs/deepinv.physics.DecomposablePhysics.md#deepinv.physics.DecomposablePhysics.prox_l2) for computing the proximal step, and thus we shouldn’t expect speed-ups in these specific cases.
...
Here we write explicitly the training loop to show how implicit differentiation can be used to avoid out-of-memory issues and sometimes accelerate training. But you can also use the [`deepinv.Trainer`](https://deepinv.org/api/stubs/deepinv.Trainer.md#deepinv.Trainer) class as shown in other examples.
...
```none
Full backpropagation: time per iteration: 1.54 s.
Implicit differentiation: time per iteration: 1.40 s.
Full backpropagation: peak memory usage: 4633.2 MB
Implicit differentiation: peak memory usage: 1671.8 MB
Memory reduction factor: 2.8x
```
...
[`Download Jupyter notebook: demo_unfolded_constant_memory.ipynb`](https://deepinv.org/auto_examples/unfolded/_downloads/c7e36bcaa21741ac5318f37ab8658137/demo_unfolded_constant_memory.ipynb)

[`Download Python source code: demo_unfolded_constant_memory.py`](https://deepinv.org/auto_examples/unfolded/_downloads/0de4d34cd0bb27e15e0b678736214c70/demo_unfolded_constant_memory.py)

[`Download zipped: demo_unfolded_constant_memory.zip`](https://deepinv.org/auto_examples/unfolded/_downloads/678fb0c5d7ffaa7d6b8f3da670b1f0b9/demo_unfolded_constant_memory.zip)
