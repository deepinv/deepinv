...
We set [`deepinv.loss.SureGaussianLoss`](https://deepinv.org/api/stubs/deepinv.loss.SureGaussianLoss.md#deepinv.loss.SureGaussianLoss) as the training loss with the `unsure=True` option.
...
#### NOTE
There are (UN)SURE losses for various noise distributions. See also [`deepinv.loss.SurePGLoss`](https://deepinv.org/api/stubs/deepinv.loss.SurePGLoss.md#deepinv.loss.SurePGLoss) for mixed Poisson-Gaussian noise.
...
We train the network using the [`deepinv.Trainer`](https://deepinv.org/api/stubs/deepinv.Trainer.md#deepinv.Trainer) class.
