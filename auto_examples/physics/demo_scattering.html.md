...
For each of the $i=1,\dots,T$ transmitters, the 2D forward model is given by the inhomogeneous Helmholtz equation (see e.g., Soubies *et al.* [[133](https://deepinv.org/user_guide/other/biblio.html.md#id19)]):
...
```none
Selected GPU 0 with 2125.25 MiB free memory
```
...
gradient evaluation (see e.g. Soubies *et al.* [[133](https://deepinv.org/user_guide/other/biblio.html.md#id19)]), and does not require storing all intermediate variables (as in standard backpropagation via PyTorch autograd).
...
Peak GPU memory usage for grad computation: 117.1 MB
Peak GPU memory usage for grad computation: 236.6 MB
...
<!-- GENERATED FROM PYTHON SOURCE LINES 313-340 -->
```Python
if device != "cpu":
...
**Total running time of the script:** (1 minutes 32.488 seconds)
