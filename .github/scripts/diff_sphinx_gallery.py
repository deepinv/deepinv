...
def get_example_dependencies(
    example_file_path: Path, core_lib_path: Path, core_lib_name: str
) -> set[Path]:
    """
    ...
    try:
                ...
                for alias in node.names:
                    if alias.name.startswith(core_lib_name):
                        ...
                        if (core_lib_path / module_path).is_dir():
                            dependencies.add(
                                core_lib_path / module_path / "__init__.py"
                            )
                        else:
        ...
        print(
            f"Warning: Could not parse {example_file_path}. Error: {e}", file=sys.stderr
        )
...
def main():
    ...
    for example_file_path in all_example_files:
        ...
        dependencies = get_example_dependencies(
            example_path, CORE_LIB_ROOT, CORE_LIB_NAME
        )
