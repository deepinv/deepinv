...
- :class:`deepinv.models.UNet` now accepts a new (opional) argument `channels_per_scale` to control the number of feature maps at each stage. Bias-free batchnorm is now possible for 3D variant (:gh:`976` by `Vicky De Ridder`_)
