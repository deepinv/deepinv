...
New Features
^^^^^^^^^^^^
- Add :func:`deepinv.utils.load_raw` to load a camera raw file as a Pytorch tensor along with a metadata dictionary containing ISP hyper-parameters ( by `Raphael Achddou`_)
