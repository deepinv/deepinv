...
* **Parameters:**
  ...
  * **apply_rescale** ([*bool*](https://docs.python.org/3.9/library/functions.html#bool)) – if `True`, map the stored values (SV) to output values according to the pydicom `apply_rescale`, default False.
    ...
    Note this is only useful when the appropriate dicom tags are present, for example in CT for [`deepinv.datasets.LidcIdriSliceDataset`](https://deepinv.org/api/stubs/deepinv.datasets.LidcIdriSliceDataset.md#deepinv.datasets.LidcIdriSliceDataset)
