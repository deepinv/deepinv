...
### *class* deepinv.loss.metric.NMSE(method='l2', \*\*kwargs)

Bases: [`MSE`](https://deepinv.org/api/stubs/deepinv.loss.metric.MSE.html.md#deepinv.loss.metric.MSE)

Normalized Mean Squared Error metric.

Calculates $\|\hat{x}-x\|_2^2/\|x\|_2^2$ where $\hat{x}=\inverse{y}$.

Normalizes MSE by the squared $\ell_2$ norm of the ground truth `x`.
