...
#### NOTE
If `reduce_overlap` is set to `"mean"` and the patch size, stride and image size are compatible, this function is the exact inverse of [`deepinv.utils.image_to_patches()`](https://deepinv.org/api/stubs/deepinv.utils.image_to_patches.md#deepinv.utils.image_to_patches). If they are not compatible, an extra cropping is needed to get the exact inverse, which can be achieved by providing the `img_size` argument.
