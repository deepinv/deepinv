...
Bases: [`BaseSDE`](https://deepinv.org/api/stubs/deepinv.sampling.BaseSDE.md#deepinv.sampling.BaseSDE)
...
* **Parameters:**
  ...
  * **solver** ([*deepinv.sampling.BaseSDESolver*](https://deepinv.org/api/stubs/deepinv.sampling.BaseSDESolver.md#deepinv.sampling.BaseSDESolver)) – the solver for solving the SDE.
  * **minus_one_one** ([*bool*](https://docs.python.org/3.9/library/functions.html#bool)) – If `True`, wrap the denoiser so that SDE states `x` in [-1, 1] are converted to [0, 1] before denoising and mapped back afterward.
    Set `True` for denoisers trained on [0, 1] (all denoisers in [`deepinv.models.Denoiser`](https://deepinv.org/api/stubs/deepinv.models.Denoiser.md#deepinv.models.Denoiser));
  ...
  * **\*args** – additional arguments for the [`deepinv.sampling.BaseSDE`](https://deepinv.org/api/stubs/deepinv.sampling.BaseSDE.md#deepinv.sampling.BaseSDE).
  * **\*\*kwargs** – additional keyword arguments for the [`deepinv.sampling.BaseSDE`](https://deepinv.org/api/stubs/deepinv.sampling.BaseSDE.md#deepinv.sampling.BaseSDE).

#### sample(x_init=None, seed=None, get_trajectory=False, \*args, \*\*kwargs)

Solve the SDE and return the sample in the $[0, 1]$ range.

Identical to [`deepinv.sampling.BaseSDE.sample()`](https://deepinv.org/api/stubs/deepinv.sampling.BaseSDE.md#deepinv.sampling.BaseSDE.sample), except that under
`minus_one_one` the sample, given in $[-1, 1]$ is mapped back to $[0, 1]$.

See [`deepinv.sampling.BaseSDE.sample()`](https://deepinv.org/api/stubs/deepinv.sampling.BaseSDE.md#deepinv.sampling.BaseSDE.sample) for the parameters.
