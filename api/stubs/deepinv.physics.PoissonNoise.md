...
When used in combination with physics operators that can produce negative outputs (such as [`deepinv.physics.BlurFFT`](https://deepinv.org/api/stubs/deepinv.physics.BlurFFT.md#deepinv.physics.BlurFFT)), it is recommended to set `clip_positive=True` to avoid runtime errors.

#### TIP
All [pretrained denoisers](https://deepinv.org/user_guide/reconstruction/denoisers.md#denoisers) in the library can be re-used for Poisson denoising
