...
* **Parameters:**
  ...
  * **noise_model** ([*deepinv.physics.NoiseModel*](https://deepinv.org/api/stubs/deepinv.physics.NoiseModel.md#deepinv.physics.NoiseModel) *,* *Callable*) – function that adds noise to the measurements $\noise{z}$.
...
* **Parameters:**
  * **solver** ([*str*](https://docs.python.org/3.9/library/stdtypes.html#str)) – solver to use. If the physics are non-linear, the only available solver is `'gradient_descent'`.
    For linear operators, the options are `'CG'`, `'lsqr'`, `'BiCGStab'` and `'minres'` (see [`deepinv.optim.linear.least_squares()`](https://deepinv.org/api/stubs/deepinv.optim.linear.least_squares.md#deepinv.optim.linear.least_squares) for more details).
...
Returns a [`deepinv.physics.StackedPhysics`](https://deepinv.org/api/stubs/deepinv.physics.StackedPhysics.md#deepinv.physics.StackedPhysics) object.
