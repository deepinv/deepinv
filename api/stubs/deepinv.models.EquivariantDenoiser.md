...
#### NOTE
We have implemented many popular geometric transforms, see [docs](https://deepinv.org/user_guide/training/transforms.md#transform). You can set the number of Monte Carlo samples by passing `n_trans`
...
See [Image transforms for equivariance & augmentations](https://deepinv.org/auto_examples/transforms-equivariance/demo_transforms.md#sphx-glr-auto-examples-transforms-equivariance-demo-transforms-py) for an example.

* **Parameters:**
  * **denoiser** (*Callable*) – Denoiser $\operatorname{D}_{\sigma}$.
  * **transform** ([*Transform*](https://deepinv.org/api/stubs/deepinv.transform.Transform.md#deepinv.transform.Transform)) – geometric transformation. If None, defaults to rotations of multiples of 90 with horizontal flips (see note above).
    See [docs](https://deepinv.org/user_guide/training/transforms.md#transform) for list of available transforms.
  * **eval_transform** ([*Transform*](https://deepinv.org/api/stubs/deepinv.transform.Transform.md#deepinv.transform.Transform)) – transformations to be used in evaluation mode. It can be used to have true Reynolds averaging at evaluation time and efficient Monte Carlo estimation at training time. If set to `None`, evaluation transformations are the same as training transformations.
