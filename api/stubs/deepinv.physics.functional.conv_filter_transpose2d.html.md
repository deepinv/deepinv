...
### deepinv.physics.functional.conv_filter_transpose2d(x, y, filter_size, padding='circular', correlation=False)

Apply the adjoint of 2D convolution with respect to its filter.

Unlike [`torch.nn.functional.conv_transpose2d()`](https://docs.pytorch.org/docs/stable/generated/torch.nn.functional.conv_transpose2d.html#torch.nn.functional.conv_transpose2d), which applies the
...
* **Parameters:**
  ...
  * **y** ([*torch.Tensor*](https://docs.pytorch.org/docs/stable/tensors.html#torch.Tensor)) – Adjoint input. Its spatial shape must match the output
    of [`deepinv.physics.functional.conv2d()`](https://deepinv.org/api/stubs/deepinv.physics.functional.conv2d.html.md#deepinv.physics.functional.conv2d) applied to `x`.
