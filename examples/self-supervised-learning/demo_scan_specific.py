...
reconstruct MRI scans from a single sample without ground truth.

Here, we demonstrate training with the :class:`SSDU loss <deepinv.loss.SplittingLoss>` :footcite:p:`yaman2020self` and :class:`weighted SSDU <deepinv.loss.WeightedSplittingLoss>` :footcite:p:`millard2023theoretical`.
...
for example see the :ref:`example using Equivariant Imaging sphx_glr_auto_examples_self-supervised-learning_demo_scan_specific.py>` :footcite:p:`chen2021equivariant`.
...
# TODO download fastmri knee singlecoil test raw kspace sample using FastMRISliceDataset
