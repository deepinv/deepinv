...
if not os.path.exists(
    f"{path_datasets}/artificial/{train_dataset_name}/dinv_dataset0.h5"
):
    ...
    train_input_dataset = datasets.ImageFolder(
        root=f"{path_datasets}/{train_dataset_name}/", transform=train_transform
    )
    test_input_dataset = datasets.ImageFolder(
        root=f"{path_datasets}/{test_dataset_name}/", transform=val_transform
    )
    dinv.datasets.generate_dataset(
        ...
        physics=p,
        device=dinv.device,
        save_dir=f"{path_datasets}/artificial/{train_dataset_name}/",
        max_datapoints=max_datapoints,
        num_workers=num_workers,
    )

train_dataset = dinv.datasets.HDF5Dataset(
    path=f"{path_datasets}/artificial/{train_dataset_name}/dinv_dataset0.h5", train=True
)
eval_dataset = dinv.datasets.HDF5Dataset(
    path=f"{path_datasets}/artificial/{train_dataset_name}/dinv_dataset0.h5",
    train=False,
)
...
params_algo = {"stepsize": [1.0], "g_param": [0.01], "lambda": 1.0}
