...
for s in [50, 10, 1, 0.5, 0.1]:

    sigmat = s * torch.tensor([1]).to(device)

    x_in = x0 + sigmat * torch.randn_like(x0)

    # apply denoiser
    x0_t = model.forward_denoise(x_in, sigmat)
    ...
    plot(
        ...
        titles=[
            "ground-truth",
            "noisy y = x + sigma*noise",
            "posterior mean with model.forward_denoise",
        ],
        figsize=(10, 5),
    )
