...
params_algo={'stepsize': 1, 'g_param': 1., 'lamb': 1}

stepsize_prox_inter = 1.
max_iter_prox_inter = 50
tol_prox_inter = 1e-3

prior = {'g': L2Prior()}
F_fn = lambda x,cur_params,y,physics : params_algo['lamb'][0]*data_fidelity.f(physics.A(x), y) + prior['g'][0](x,cur_params['g_param'])
model = Optim(algo_name = 'PGD', prior=prior, g_first = False, data_fidelity=data_fidelity,
             params_algo=params_algo, early_stop = early_stop, max_iter=max_iter, crit_conv=crit_conv, 
             thres_conv=thres_conv, backtracking=True, F_fn=F_fn, return_dual=False, verbose=True, return_metrics = plot_metrics,
             stepsize_prox_inter = stepsize_prox_inter, max_iter_prox_inter = max_iter_prox_inter, tol_prox_inter = tol_prox_inter)
