...
p = dinv.physics.Downsampling(img_size = (n_channels,img_size,img_size), factor=factor, filter=filter_torch, device=dinv.device, noise_model = dinv.physics.GaussianNoise(sigma=noise_level_img))
data_fidelity = L2()
...
lamb, sigma_denoiser, stepsize, max_iter, alpha_denoiser = get_ProxPnP_params(algo_name, noise_level_img)
...
model = Optim(algo_name = algo_name, prior=prior, g_first = False, data_fidelity=data_fidelity, 
             ...
             F_fn=None, return_dual=False, verbose=True)
