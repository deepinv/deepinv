...
.. note::
    This example requires `DIRECT <https://docs.aiforoncology.nl/direct/>`_. Install with `pip install direct-recon`.
...
dinv.datasets.download_archive(dinv.utils.get_image_url("CMRxRecon.zip"), dinv.utils.get_cache_home() / "CMRxRecon.zip", extract=True)
...
dinv.utils.plot({f"y of shape {tuple(y.shape)}": y, f"mask of acc {1 / mask.mean().item():.2f}": mask}, figsize=(6, 8), suptitle="CMRxRecon data")
...
vsharp = dinv.models.DIRECTModel(model_name="vsharp_cardiac", pretrained=True, device=device)
...
dinv.utils.plot({"Fully-sampled": x, "Zero-filled": x_zf, "vSHARP": x_vsharp, "RAM": x_ram}, subtitles=[f"{metric(x).item():.1f}", f"{metric(x_zf).item():.1f}", f"{metric(x_vsharp).item():.1f}", f"{metric(x_ram).item():.1f}"])

# %%
# Multicoil brain MRI reconstruction
# ----------------------------------
#
# We use an accelerated multicoil Calgary-Campinas brain test volume with no ground truth,
# with :class:`ram_experiments.datasets.calgary.CalgarySliceDataset`, which loads undersampled y, mask, and estimated coil maps using ESPIRiT.
# Take a single slice for the demo, and construct a 2D :class:`deepinv.physics.MultiCoilMRI` physics.

# TODO download calgary from HF

dataset = dinv.datasets.CalgarySliceDataset(
    "/Volumes/E/ram-experiments/data/calgary/calgary_test_5",
    slice_index="middle",
    transform=dinv.datasets.CalgarySliceTransform(estimate_coil_maps=True, acs=24, espirit_crop=0.85),
)
...
dinv.utils.plot({"Mask": physics.mask, "0th coil map": physics.coil_maps[:, [0]]}, figsize=(6, 8))

# %%
# Perform reconstruction with pretrained models.
# Note that vSHARP and Joint-ICNet (TODO CITE) estimate coil maps internally, whereas RAM uses the ESPIRiT maps.

vsharp = dinv.models.DIRECTModel(model_name="vsharp_brain", pretrained=True, device=device)
jointicnet = dinv.models.DIRECTModel(model_name="jointicnet_5x", pretrained=True, device=device)
ram = dinv.models.RAM(device=device, pretrained=True)
...
dinv.utils.plot({"Zero-filled": x_zf, "vSHARP": x_vsharp, "Joint-ICNet": x_jointicnet, "RAM": x_ram}, subtitles=[f"{metric(x_zf).item():.1f}", f"{metric(x_vsharp).item():.1f}", f"{metric(x_jointicnet).item():.1f}", f"{metric(x_ram).item():.1f}"])
...
# TODO download fastmri test+full from HF

dataset = dinv.datasets.FastMRISliceDataset(
    "/Volumes/E/ram-experiments/data/fastMRI/brain_multicoil/multicoil_test",
    target_root="/Volumes/E/ram-experiments/data/fastMRI/brain_multicoil/multicoil_test_full",
    slice_index="middle",
    filter_id=lambda s: "file_brain_AXT2_200_2000341" in str(s.fname),
    transform=dinv.datasets.MRISliceTransform(estimate_coil_maps=True, espirit_crop=0.85),
)
