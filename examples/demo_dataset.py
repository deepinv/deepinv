...
data_train = datasets.MNIST(
    root="../datasets/", train=True, transform=transform_data, download=True
)
data_test = datasets.MNIST(root="../datasets/", train=False, transform=transform_data)

# choose forward operator
physics = dinv.physics.Inpainting(tensor_size=(1, 28, 28), mask=0.5, device=dinv.device)
physics.noise_model = dinv.physics.GaussianNoise(sigma=0.1)  # add Gaussian Noise
