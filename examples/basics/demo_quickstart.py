...
physics = dinv.physics.MRI(**dinv.physics.generator.RandomMaskGenerator(x.shape[2:]).step())

# Inpainting with noise model
physics = dinv.physics.Inpainting(x.shape[1:], mask=0.8, noise_model=dinv.physics.GaussianNoise(0.1))
...
model = dinv.optim.optim_builder(
    iteration="HQS", prior=prior, data_fidelity=data_fidelity,
