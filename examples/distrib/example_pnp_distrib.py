...
def run_single_process_pnp(
    ...
    with torch.no_grad():
        ...
        for it in range(num_iterations):
            ...
            if it == 0 or (it + 1) % 5 == 0:
                print(f"  Single-process PnP iteration {it+1}/{num_iterations}, PSNR: {psnr_val:.2f} dB")
...
def run_distributed_pnp_example():
    ...
    with DistributedContext(sharding="round_robin", seed=42) as ctx:
        ...
        drunet_prior = PnP(denoiser=dinv.models.DRUNet(pretrained="download").to(ctx.device))
        ...
        if ctx.rank == 0:
            ...
            print(f"  ✅ Distributed prior: tiling with {patch_size}x{patch_size} patches")
        ...
        with torch.no_grad():
            for it in range(num_iterations):
                ...
                if ctx.rank == 0:
                    psnr_val = psnr_metric(distributed_signal.data, clean_image.unsqueeze(0)).item()
                    psnr_history_distributed.append(psnr_val)
                    
                    if it == 0 or (it + 1) % 5 == 0:
                        print(f"  Distributed PnP iteration {it+1}/{num_iterations}, PSNR: {psnr_val:.2f} dB")
        ...
        if ctx.rank == 0:
            ...
            print(f"  • PSNR difference: {abs(psnr_history_distributed[-1] - single_psnr_history[-1]):.4f} dB")
...
def save_results_visualization(
    ...
    if pinv_recon.shape != clean_image.shape:
        pinv_recon = torch.nn.functional.interpolate(
            pinv_recon.unsqueeze(0), size=clean_image.shape[-2:], mode='bilinear', align_corners=False
        ).squeeze(0)
    ...
    plt.plot(iterations, psnr_distributed, 'b-', linewidth=2, 
             label=f'Distributed (World Size: {ctx.world_size})')
    plt.plot(iterations, psnr_single, 'r--', linewidth=2, 
             label='Single-process')
    plt.axhline(y=psnr_pinv, color='g', linestyle=':', linewidth=2, 
    ...
    for bar, psnr in zip(bars, psnrs):
        ...
        plt.text(bar.get_x() + bar.get_width()/2., height + 0.1,
                f'{psnr:.2f}', ha='center', va='bottom', fontsize=10)
