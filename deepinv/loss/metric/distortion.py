    ...
    def metric(self, x_net: Tensor = None, x: Tensor = None, *args, **kwargs) -> Tensor:
        ...
        if is_color_image:
            coefficients_ref_i = torch.abs(
                self._convolve2d(ref_i, torch.ones((2, 2), device=ref_i.device) / 4.0)
            )
            coefficients_deg_i = torch.abs(
                self._convolve2d(deg_i, torch.ones((2, 2), device=deg_i.device) / 4.0)
            )
            coefficients_ref_q = torch.abs(
                self._convolve2d(ref_q, torch.ones((2, 2), device=ref_q.device) / 4.0)
            )
            coefficients_deg_q = torch.abs(
                self._convolve2d(deg_q, torch.ones((2, 2), device=deg_q.device) / 4.0)
            )
