...
from deepinv.loss.metric.functional import cal_psnr, complex_abs, cal_mse
...
def import_pyiqa() -> ModuleType:
    try:
        ...
        raise ImportError("Metric not available. Please install the pyiqa package with `pip install pyiqa`.")
...
class Metric(Loss):
    ...
    def metric(self, x_net: torch.Tensor, x: torch.Tensor, *args, **kwargs) -> torch.Tensor:
    ...
    def forward(self, x_net, x, y, physics, model, **kwargs):
        ...
        m = 1. - m if self.train_loss else m
        return m
