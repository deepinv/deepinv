...
class Metric(Module):
    ...
    :param str norm_inputs: normalize images before passing to metric. ``l2``normalizes by L2 spatial norm, ``min_max`` normalizes by min and max of each input, ``clip`` clips to :math:`[0,1]`, ``standardize`` standardizes using mean and std of ground truth, ``none`` or None no reduction will be applied (default).
    ...
    def __init__(
        ...
        if norm_inputs is not None:
            if not isinstance(norm_inputs, str):
                raise ValueError(
                    "norm_inputs must be str or None."
                )
        ...
        if reduction is not None:
            if not isinstance(reduction, str):
                raise ValueError("reduction must either be str or None.")
