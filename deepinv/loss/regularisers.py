...
class JacobianSpectralNorm(nn.Module):
    ...
    def forward(self, y, x):
        ...
        for it in range(self.max_iter):
            ...
            v = torch.autograd.grad(torch.autograd.grad(y, x, w, create_graph=True),
                                    w, u, create_graph=not self.eval)[0]  # v = A(u)
            ...
            if it > 0:
                ...
                if rel_var < self.tol and self.verbose:
                    print("Power iteration converged at iteration: ", it, ", val: ", z.sqrt().item(), ", relvar :",
                          rel_var)
                    break
            zold = z.detach().clone()
...
class FNEJacobianSpectralNorm(nn.Module):
    ...
    def __init__(self, max_iter=100, tol=1e-3, verbose=False, eval_mode=False):
        ...
        self.spectral_norm_module = JacobianSpectralNorm(max_iter=max_iter, tol=tol, verbose=verbose,
                                                         eval_mode=eval_mode)

    def forward(self, y_in, x_in, model, *args_model, interpolation=False, **kwargs_model):
        r'''
