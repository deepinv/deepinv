...
class SplittingLoss(Loss):
    ...
    Implements measurement splitting loss from :cite:p:`yaman2020self` (SSDU) for MRI,
    :cite:p:`hendriksen2020noise2inverse` (Noise2Inverse) for CT,
    :cite:p:`acar2021self` for dynamic MRI. Also see :class:`deepinv.loss.Artifact2ArtifactLoss`, :class:`deepinv.loss.Phase2PhaseLoss` for similar.
    ...
    This loss was used in SSDU for MRI in :cite:p:`yaman2020self`.
    ...
    .. note::
        ...
        To disable measurement splitting (and use the full input) at evaluation time, set ``eval_split_input=False``. This is done in SSDU :cite:p:`yaman2020self`.
...
class Phase2PhaseLoss(SplittingLoss):
    ...
    Implements dynamic measurement splitting loss from :cite:p:`eldeniz2021phase2phase`.
...
class Artifact2ArtifactLoss(Phase2PhaseLoss):
    ...
    Implements dynamic measurement splitting loss from :cite:p:`liu2020rare`.
    for free-breathing MRI.
...
class Neighbor2Neighbor(Loss):
    r"""
    Neighbor2Neighbor loss :cite:p:`huang2021neighbor2neighbor`.

    Splits the noisy measurements using two masks :math:`A_1` and :math:`A_2`, each choosing a different neighboring
    map (see details in :cite:p:`huang2021neighbor2neighbor`). The self-supervised loss is computed as:
