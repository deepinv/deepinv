...
class NeuralIteration(nn.Module):
    ...
    def init(self, backbone_blocks, step_size=1.0, iterations=1):
        ...
        self.register_parameter(name='step_size',
                                param=torch.nn.Parameter(step_size * torch.ones(self.iterations),
                                                         requires_grad=True))
        if self.n_blocks > 1:  # weight_tied=False (many blocks)
            assert self.n_blocks == iterations, "'# blocks' does not equal to 'iterations'"
            self.blocks = torch.nn.ModuleList([backbone_blocks[_] for _ in range(iterations)])
    ...
    def measurement_consistency_grad(physics, x, y):
        # grad(||y-Ax||) = A^T(y-Ax)
        return physics.A_adjoint(y.to(physics.device) - physics.A(x.to(physics.device))).to(x.device)
...
class GradientDescent(NeuralIteration):
    ...
    def forward(self, y, physics, x_init=None):
        ...
        for t in range(self.iterations):
            if self.n_blocks==1:
                t=0
            x = self.blocks[t](x) + x + self.step_size[t] * NeuralIteration.measurement_consistency_grad(physics, x, y)
        return x
...
class ProximalGradientDescent(NeuralIteration):
    ...
    def forward(self, y, physics, x_init=None):
        ...
        for t in range(self.iterations):
            if self.n_blocks==1:
                t=0
            x = self.blocks[t](x + self.step_size[t] * NeuralIteration.measurement_consistency_grad(physics, x, y))
        return x
