...
class LinearSpline(nn.Module):
    ...
    def forward(self, sigma: torch.Tensor) -> torch.Tensor:
        ...
        if type(sigma) is not torch.Tensor and type(sigma) is not float:
            raise TypeError(f"Expected sigma to be a 1D torch.Tensor or float, got {type(sigma)}")
        elif type(sigma) is float:
            sigma = torch.tensor([sigma], dtype=torch.float32, device=self.s_at_knots.device)
