...
def patch_extractor(imgs,n_patches,patch_size,duplicates=False,position_inds_linear=None):
    """
    ...
    patch=torch.zeros((C,patch_size,patch_size),dtype=torch.long,device=imgs.device)
    ...
    if position_inds_linear is None:
        if duplicates:
            position_inds_linear=torch.randint(0,total_patch_number,(n_patches,),device=imgs.device)
        else:
            position_inds_linear=torch.randperm(total_patch_number,device=imgs.device)[:n_patches]
