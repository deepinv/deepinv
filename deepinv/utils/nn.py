...
def get_freer_gpu(verbose=True):
    ...
    """
    try:
        os.system("nvidia-smi -q -d Memory |grep -A5 GPU|grep Free >tmp" if os.name == "posix" else 'bash -c "nvidia-smi -q -d Memory |grep -A5 GPU|grep Free >tmp"')
        ...
        if torch.cuda.device_count() == 0:
            ...
            idx, mem = max(
                ((d, torch.cuda.mem_get_info(d)[0] / 1048576) for d in range(torch.cuda.device_count())),
                key=lambda x: x[1]
            )
            device = torch.device(f"cuda:{idx}")
