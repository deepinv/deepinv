...
@pytest.mark.parametrize("prior_name", LIST_PRIORS)
@pytest.mark.parametrize("pnp_algo", ["PGD", "HQS", "DRS", "ADMM", "CP", "FISTA"])
def test_priors_algo(pnp_algo, imsize, dummy_dataset, device, prior_name):
    # 1. Generate a dummy dataset
    dataloader = DataLoader(dummy_dataset, batch_size=1, shuffle=False, num_workers=0)
    test_sample = next(iter(dataloader)).to(device)
...
@pytest.mark.parametrize("prior_name", LIST_PRIORS)
def test_priors(imsize, prior_name, device):

    # here the prior model is common for all iterations
    prior = get_prior(prior_name, device=device)

    batch_size = 4
    x = torch.randn((batch_size, *imsize)).to(device)

    # test that the prior has a right shape
    assert prior(x).shape == (batch_size,)
