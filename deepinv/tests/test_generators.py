...
GENERATORS = ["MotionBlurGenerator", "DiffractionBlurGenerator"]

MIXTURES = list(itertools.combinations(GENERATORS, 2))

SIZES = [(5, 5), (3, 5, 5), (2, 3, 5, 5)]
...
def find_mixture(name_tuple, filter_shape, device):
    ...
    for name in name_tuple:
        if name == "MotionBlurGenerator":
            gen_list.append(
                dinv.physics.MotionBlurGenerator(shape=filter_shape, device=device)
            )
        elif name == "DiffractionBlurGenerator":
            gen_list.append(
                dinv.physics.DiffractionBlurGenerator(shape=filter_shape, device=device)
            )
        else:
            raise Exception("The generator chosen doesn't exist")
