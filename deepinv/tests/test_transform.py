...
def test_reflect_manual_inverse_params():
    # Reflection is its own inverse, including when deterministic params are
    # supplied directly instead of generated by Reflect.get_params().
    x = torch.randn(1, 3, 8, 8)
    transform = dinv.transform.Reflect()
    params = {"dims": torch.tensor([[-1]])}

    x_flipped = transform.transform(x, **params)
    x_restored = transform.inverse(x_flipped, **params)

    assert torch.allclose(x_restored, x)
    assert torch.equal(transform.invert_params(params)["dims"], params["dims"])


def test_shift_axis_direction():
    # x_shift moves along image width, y_shift moves along image height.
    x = torch.zeros(1, 1, 3, 4)
    x[..., 1, 1] = 1
    transform = dinv.transform.Shift()

    shifted_x = transform.transform(x, x_shift=[1], y_shift=[0])
    shifted_y = transform.transform(x, x_shift=[0], y_shift=[1])

    expected_x = torch.zeros_like(x)
    expected_x[..., 1, 2] = 1
    expected_y = torch.zeros_like(x)
    expected_y[..., 2, 1] = 1

    assert torch.equal(shifted_x, expected_x)
    assert torch.equal(shifted_y, expected_y)
