...
def run_distributed_test(test_func, dist_config, test_args=None):
    ...
    missing_ranks = sorted(set(range(world_size)) - set(results_by_rank) - set(errors_by_rank))
    ...
    if timed_out or missing_ranks or errors_by_rank or failed_ranks:
        lines = []
        if timed_out:
            lines.append(f"Timed out after {timeout:.1f}s (received {received}/{world_size} rank results).")
        if missing_ranks:
...
class TestDistributedMeasurements:
    ...
    def test_initialization_with_list(self, dist_config):
        ...
        results = run_distributed_test(_test_measurements_initialization_with_list, dist_config)
    ...
    def test_initialization_with_factory(self, dist_config):
        ...
        results = run_distributed_test(_test_measurements_initialization_with_factory, dist_config)
...
class TestDistributedLinearPhysics:
    ...
    def test_initialization_with_list(self, dist_config):
        ...
        results = run_distributed_test(_test_physics_initialization_with_list, dist_config)
...
class TestDistributedSignal:
    ...
    def test_initialization_and_sync(self, dist_config):
        ...
        results = run_distributed_test(_test_signal_initialization_and_sync, dist_config)
...
class TestFactoryAPI:
    ...
    def test_make_distrib_bundle_basic(self, dist_config):
        ...
        results = run_distributed_test(_test_factory_make_distrib_bundle_basic, dist_config)
        assert all(r["success"] for r in results)

    def test_make_distrib_bundle_with_prior(self, dist_config):
        """Test make_distrib_bundle with prior."""
        results = run_distributed_test(_test_factory_make_distrib_bundle_with_prior, dist_config)
        assert all(r["success"] for r in results)
