...
@pytest.mark.parametrize("gain", [.05, .5])
def test_anscombe_transform(sigma, gain, device, rng, load_example_image):
    x = torch.ones((1, 1, 16, 16), device=device)
    physics = dinv.physics.Denoising(dinv.physics.PoissonGaussianNoise(sigma=sigma, gain=gain))
    y = physics(x)
    z = dinv.models.generalized_anscombe_transform(y, sigma=sigma, gain=gain)

    # std(GAT(y)) \approx gain
    assert torch.allclose(z.std(), torch.tensor(gain, device=device), atol=0.05, rtol=0.05)

    # IGAT(GAT(y))\approx y
    y_inv = dinv.models.inverse_generalized_anscombe_transform(z, sigma=sigma, gain=gain)
    ...
    with torch.no_grad():
        x_base = denoiser(y, sigma=(sigma**2 + gain)**(.5))
