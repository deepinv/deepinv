...
def choose_loss(loss_name):
    ...
    if loss_name == 'mcei':
        ...
        loss.append(dinv.loss.SplittingLoss(regular_mask=True, split_ratio=.25))
        loss.append(dinv.loss.TVLoss())
    elif loss_name == 'score':
        loss.append(dinv.loss.ScoreLoss(1.))
    elif loss_name == 'sup':
...
def physics(imsize):
    ...
    return dinv.physics.Inpainting(tensor_size=imsize, mask=.5, device=dinv.device)
...
def dataset(physics, tmp_path, imsize):
    ...
    dinv.datasets.generate_dataset(train_dataset=DummyCircles(samples=50, imsize=imsize),
                                   test_dataset=DummyCircles(samples=10, imsize=imsize),
                                   physics=physics, save_dir=save_dir, device=dinv.device)

    return dinv.datasets.HDF5Dataset(save_dir / 'dinv_dataset0.h5', train=True),\
...
def test_losses(loss_name, tmp_path, dataset, physics, imsize):
    ...
    save_dir = tmp_path / 'dataset'
    # choose backbone denoiser
    backbone = dinv.models.AutoEncoder(dim_input=imsize[0]*imsize[1]*imsize[2], dim_mid=128, dim_hid=32).to(dinv.device)
    ...
    initial_psnr = dinv.test(model=model, test_dataloader=test_dataloader, physics=physics,
                             plot=False, device=dinv.device)

    # train the network
    model = dinv.train(model=model, train_dataloader=dataloader, epochs=epochs, scheduler=scheduler, losses=loss,
                       physics=physics, optimizer=optimizer, device=dinv.device, ckp_interval=int(epochs / 2),
                       save_path=save_dir / 'dinv_test', plot=False, verbose=False)

    final_psnr = dinv.test(model=model, test_dataloader=test_dataloader, physics=physics, plot=False, device=dinv.device)
