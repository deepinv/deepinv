...
class BaseOptim(Reconstructor):
    ...
    def update_prior_fn(self, it):
        ...
        """
        if isinstance(self.fixed_point.iterator, MACEIteration):
            return self.prior
        else:
...
def create_iterator(
    ...
    if isinstance(
        ...
        # For MACE, pass the full lists of data_fidelity and prior objects
        if iteration == "MACE":
            return iterator_fn(
                data_fidelity_list=data_fidelity,
                prior_list=prior,
                F_fn=None,
                has_cost=False,
            )
...
def optim_builder(
    ...
    **kwargs,
    ...
    :param str, deepinv.optim.OptimIterator iteration: either the name of the algorithm to be used,
        ...
        ``"HQS"`` (half-quadratic splitting), ``"CP"`` (Chambolle-Pock), ``"DRS"`` (Douglas Rachford)
        or ``"MACE"`` (Multi-Agent Consensus Equilibrium).
    :param int max_iter: maximum number of iterations of the optimization algorithm. Default: 100.
    :param dict params_algo: dictionary containing all the relevant parameters for running the algorithm,
                            e.g. the stepsize, regularization parameter, denoising standard deviation.
                            Each value of the dictionary can be either Iterable (distinct value for each iteration) or
