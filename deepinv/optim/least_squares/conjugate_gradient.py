...
def conjugate_gradient(
    ...
    for iteration in range(int(max_iter)):
        if not torch.any(active):
            if verbose:
                print("CG Converged at iteration", iteration)
            break

        Ap = A(p)
        pAp = dot(p, Ap, dim=dim).real

        alpha = torch.where(active, res_old / pAp, torch.zeros_like(res_old))
        ...
        if iteration > 0 and iteration % 50 == 0 and torch.any(active):
            r_true = b - A(x)
            r = torch.where(active.unsqueeze(-1).expand_as(r), r_true, r)
            res_old = dot(r, r, dim=dim).real
    else:
        if verbose:
