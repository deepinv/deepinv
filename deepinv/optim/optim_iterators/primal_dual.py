...
class fStepPD(fStep):
    ...
    def forward(
        self, Ax_cur, u, y, cur_params
    ):  # Beware this is not the prox of f(A\cdot) but only the prox of f, A is tackled independently in PD
...
class gStepPD(gStep):
    ...
    def forward(self, x, Atu, prior, cur_params):
        ...
        )  # Beware, this is not correct: we should have a product of stepsize / reg param
