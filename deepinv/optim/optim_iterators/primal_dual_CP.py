...
class CPIteration(OptimIterator):
    ...
    Class for a single iteration of the Chambolle-Pock Primal-Dual (PD) algorithm :cite:`chambolle2011first` for minimising :math:`F(Kx) + \lambda G(x)` or :math:`\lambda F(x) + G(Kx)` for generic functions :math:`F` and :math:`G`.
    Our implementation corresponds to Algorithm 1 of :cite:`chambolle2011first`.
