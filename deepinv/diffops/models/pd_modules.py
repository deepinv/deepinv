...
class DualBlock(nn.Module):
    def __init__(self, in_channels=3, out_channels=1, depth=3, bias=True, nf=5):
        ...
        self.in_conv = nn.Conv1d(in_channels, nf, kernel_size=3, stride=1, padding=1, bias=bias)
        self.conv_list = nn.ModuleList(
            [nn.Conv1d(nf, nf, kernel_size=3, stride=1, padding=1, bias=bias) for _ in range(self.depth - 2)])
        self.out_conv = nn.Conv1d(nf, out_channels, kernel_size=3, stride=1, padding=1, bias=bias)
