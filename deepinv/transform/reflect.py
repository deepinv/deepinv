...
class Reflect(Transform):
    ...
    def _get_params(self, x: torch.Tensor) -> dict:
        ...
        idx = torch.randperm(len(subsets), generator=self.rng, device=self.rng.device)[: self.n_trans]
        out = torch.full((self.n_trans, len(self.dim)), fill_value=float("nan"), device=x.device)
