...
class ShiftTime(Transform):
    ...
    def roll_reflect_1d(self, x: torch.Tensor, by=0, dim=0):
        def arange(*args): return torch.arange(*args, device=x.device)
        ...
        return torch.roll(x_pad, [by], [dim]).index_select(dim, arange(T - 1, T * 2 - 1))
    ...
    def _transform(self, x: torch.Tensor, amounts: Union[torch.Tensor, Iterable, TransformParam] = [], **kwargs) -> torch.Tensor:
        roll = torch.roll if self.padding == "wrap" else self.roll_reflect_1d
        return torch.cat(
            [roll(x, by.item() if isinstance(by, torch.Tensor) else by, -3) for by in amounts],
            dim=0,
        )