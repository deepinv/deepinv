...
try:
    cpab_cpu = load(
        name="cpab_cpu",
        sources=[_dir + "/c/transformer.cpp", _dir + "/c/cpab_ops.cpp"],
        verbose=_verbose,
    )
    _cpu_succes = True
    if _verbose:
...
def CPAB_transformer_slow(points, theta, params):
    ...
    newpoints = torch.reshape(
        newpoints, (-1, params.ndim + 1)
    )  # [n_theta*n_points, ndim+1]]
    ...
    batch_idx = params.nC * (
        torch.ones(n_points, n_theta, dtype=torch.int64) * torch.arange(n_theta)
    )
...
def CPAB_transformer_fast(points, theta, params):
    if params.numeric_grad:
        return _CPABFunction_NumericGrad.apply(points, theta, params)
    else:
        return _CPABFunction_AnalyticGrad.apply(points, theta, params)
...
class _CPABFunction_AnalyticGrad(torch.autograd.Function):
    ...
    def backward(ctx, grad):  # grad [n_theta, ndim, n]
        ...
        return (
            ...
            g.t(),
            None,
        )  # transpose, since pytorch expects a [n_theta, d] matrix
