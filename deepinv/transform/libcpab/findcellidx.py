...
def findcellidx(ndim, p, nc):
    if ndim == 1:
        return findcellidx1D(p, *nc)
    elif ndim == 2:
        return findcellidx2D(p, *nc)
    elif ndim == 3:
        return findcellidx3D(p, *nc)
...
def findcellidx2D(p, nx, ny):
    ...
    idx = (
        mymin((nx - 1) * torch.ones(n, device=p.device), (p0 - xmod) / inc_x)
    ...
    cond2 = (
        (~cond1)
        & (p[0] <= 0)
        & ((p[1] >= ny * inc_y) & (p[1] / inc_y - ny > -p[0] / inc_x))
    )
    ...
    cond4 = (
        (~out)
        & (p[0] >= nx * inc_x)
        & ((p[1] <= 0) & (-p[1] / inc_y > p[0] / inc_x - nx))
    )
    cond5 = (
        (~out)
        & (~cond4)
        & (p[0] >= nx * inc_x)
        & ((p[1] >= ny * inc_y) & (p[1] / inc_y - ny > p[0] / inc_x - nx))
    )
...
def findcellidx3D(p, nx, ny, nz):
    ...
    cond_y = abs_y > half
    cond_z = abs_z > half
    points_outside[0, cond_x] = points_outside[0, cond_x].sign() * (
        half - push_x[cond_x]
    )
    points_outside[1, cond_y] = points_outside[1, cond_y].sign() * (
        half - push_y[cond_y]
    )
    points_outside[2, cond_z] = points_outside[2, cond_z].sign() * (
        half - push_z[cond_z]
    )
    points_outside += half
    ...
    i = mymin(
        torch.tensor(nx - 1).to(torch.float32).to(p.device), ((p0 - xmod) / inc_x)
    )
    j = mymin(
        torch.tensor(ny - 1).to(torch.float32).to(p.device), ((p1 - ymod) / inc_y)
    )
    k = mymin(
        torch.tensor(nz - 1).to(torch.float32).to(p.device), ((p2 - zmod) / inc_z)
    )
