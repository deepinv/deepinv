...
class Cpab(object):
    ...
    def __init__(
        ...
        if not (0 < len(tess_size) <= 3) or not all([e > 0 for e in tess_size]):
            raise ValueError(
                "tess_size must be a list of positive integers of length 1, 2 or 3."
            )
        ...
        self.params.inc = [
            (self.params.domain_max[i] - self.params.domain_min[i]) / self.params.nc[i]
    ...
    def uniform_meshgrid(self, n_points, domain_min=None, domain_max=None):
        ...
        if domain_min is None:
            domain_min = self.params.domain_min
        if domain_max is None:
            domain_max = self.params.domain_max
        lin = [
            torch.linspace(
                domain_min[i], domain_max[i], n_points[i], device=self.device
            )
            for i in range(self.params.ndim)
        ]
    ...
    def sample_transformation(self, n_sample=1, mean=None, cov=None):
        ...
        """
        mean = (
            torch.zeros(self.params.d, dtype=torch.float32, device=self.device)
            if mean is None
            else mean
        )
        cov = (
            torch.eye(self.params.d, dtype=torch.float32, device=self.device)
            if cov is None
            else cov
        )
    ...
    def sample_transformation_with_prior(
        self, n_sample=1, mean=None, length_scale=0.1, output_variance=1
    ):
        ...
        centers = torch.tensor(
            self.tesselation.get_cell_centers(), dtype=torch.float32, device=self.device
        )
    ...
    def identity(self, n_sample=1, epsilon=0):
        ...
        return (
            torch.zeros(
                n_sample, self.params.d, dtype=torch.float32, device=self.device
            )
            + epsilon
        )
    ...
    def transform_grid(self, grid, theta):
        ...
        if len(grid.shape) == 3:  # check that grid and theta can broadcastes together
            assert (
                grid.shape[0] == theta.shape[0]
            ), """When passing a 3D grid, expects
    ...
    def calc_vectorfield(self, grid, theta):
        ...
        grid = torch.cat(
            (grid, torch.ones(1, grid.shape[1], device=grid.device)), dim=0
        )
    ...
    def visualize_deformgrid(
        self, theta, nb_lines=10, nb_points=1000, fig=plt.figure()
    ):
    ...
    def visualize_tesselation(self, nb_points=50, show_outside=False, fig=plt.figure()):
        ...
        if show_outside:
            domain_size = [
                self.params.domain_max[i] - self.params.domain_min[i]
                for i in range(self.params.ndim)
            ]
            domain_min = [
                self.params.domain_min[i] - domain_size[i] / 10
                for i in range(self.params.ndim)
            ]
            domain_max = [
                self.params.domain_max[i] + domain_size[i] / 10
                for i in range(self.params.ndim)
            ]
            grid = self.uniform_meshgrid(
                [nb_points for _ in range(self.params.ndim)], domain_min, domain_max
            )
        else:
            grid = self.uniform_meshgrid([nb_points for _ in range(self.params.ndim)])
