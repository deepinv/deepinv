...
class Tesselation(object):
    ...
    def __init__(
        ...
        self._basis_file = (
            ...
            + str(len(self.nc))
            + "_tess"
            + "_".join([str(e) for e in self.nc])
            + "_"
            + "vo"
            + str(int(not self.zero_boundary))
            + "_"
            + "zb"
            + str(int(self.zero_boundary))
            + "_"
            + "vp"
            + str(int(self.volume_perservation))
        )
        ...
        if not os.path.isfile(self._basis_file + ".cpab_basis") or override:
            ...
            null_mask = np.concatenate(
                ((s <= 1e-6), np.ones((padding,), dtype=bool)), axis=0
            )
        ...
        for idx, (i, j) in enumerate(self.shared_v_idx):
            for vidx in range(self.ndim):
                for k in range(self.ndim):
                    ...
                    row[0, index2 : index2 + (self.ndim + 1)] = -self.shared_v[idx][
                        vidx
                    ]
    ...
    def create_zero_trace_constrains(self):
        ...
        row = np.concatenate(
            (np.eye(self.ndim), np.zeros((self.ndim, 1))), axis=1
        ).flatten()
...
class Tesselation1D(Tesselation):
    def __init__(
        ...
        super(Tesselation1D, self).__init__(
            nc,
            domain_min,
            domain_max,
            zero_boundary,
...
class Tesselation2D(Tesselation):
    def __init__(
        ...
        # Initialize super class
        super(Tesselation2D, self).__init__(
            nc,
            domain_min,
            domain_max,
            zero_boundary,
    ...
    def find_verts_outside(self):
        ...
        for i in range(self.nC):
            for j in range(self.nC):
                ...
                if (
                    len(shared_verts) == 1
                    and any([left[i, j], right[i, j], top[i, j], bottom[i, j]])
                    and (j, i) not in shared_v_idx
                ):
...
class Tesselation3D(Tesselation):
    def __init__(
        ...
        # Initialize super class
        super(Tesselation3D, self).__init__(
            nc,
            domain_min,
            domain_max,
            zero_boundary,
            ...
            for j in range(self.nC):
                if i != j:
                    ...
                    for d in range(self.ndim):
                        ...
                        upper_cond = (
                            sum(vi[:, d] == self.domain_min[d]) == 3
                            and sum(vj[:, d] == self.domain_min[d]) == 3
                        )
                        lower_cond = (
                            sum(vi[:, d] == self.domain_max[d]) == 3
                            and sum(vj[:, d] == self.domain_max[d]) == 3
                        )
                        dist_cond = (
                            sum([abs(i1 - i2) for i1, i2 in zip(ci[:3], cj[:3])]) == 0
                        )  # same cell
    ...
    def create_zero_boundary_constrains(self):
        ...
        for c in range(self.nC):
            for v in self.verts[c]:
                ...
                if v[1] == ymin or v[1] == ymax:
                    row = np.zeros(shape=(12 * self.nC))
                    row[(12 * c) : (12 * (c + 1))] = np.concatenate(
                        [np.zeros((4,)), v, np.zeros((4,))]
                    )
                    Ltemp = np.vstack((Ltemp, row))
...
if __name__ == "__main__":
    ...
    tess2 = Tesselation2D(
        [2, 2], [0, 0], [1, 1], zero_boundary=False, volume_perservation=True
    )
    tess3 = Tesselation3D(
        [2, 2, 2], [0, 0, 0], [1, 1, 1], zero_boundary=True, volume_perservation=False
    )
