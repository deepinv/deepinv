...
def sample_from(values, shape=(1,), dtype=torch.float32, device="cpu", generator: torch.Generator = None):
    ...
    indices = (
        torch.floor(N * torch.rand(shape, dtype=dtype, device=generator.device, generator=generator))
        .to(torch.long)
        .to(device)
    )
...
class Scale(Transform):
    ...
    def _get_params(self, x: torch.Tensor) -> dict:
        ...
        center = torch.rand((b, 2), dtype=x.dtype, device=self.rng.device, generator=self.rng).to(x.device)
