...
class Shift(Transform):
    ...
    def _get_params(self, x: torch.Tensor) -> dict:
        ...
        x_shift = (
            torch.arange(-H_max, H_max, device=self.rng.device)[torch.randperm(2 * H_max, generator=self.rng, device=self.rng.device)][
                : self.n_trans
            ]
            if H_max > 0
        ...
        y_shift = (
            torch.arange(-W_max, W_max, device=self.rng.device)[torch.randperm(2 * W_max, generator=self.rng, device=self.rng.device)][
                : self.n_trans
            ]
            if W_max > 0
            else torch.zeros(self.n_trans, device=x.device)
