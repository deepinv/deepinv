...
def test(model, test_dataloader, physics, device=torch.device(f"cuda:0"), plot_images=False, plot_input=True,
         save_folder='results', plot_metrics=False, verbose=True, wandb_vis=False, **kwargs):
    r'''
        ...
        for i, (x, y) in enumerate(tqdm(dataloader)):
            ...
            if plot_images : 
                if g < show_operators :
                    ...
                    for img, name_im in zip(imgs, name_imgs):
                        im_save(os.path.join(save_folder_G, name_im + '_' + str(i) + '.png'), img)
                    if wandb_vis:
                        n_plot = min(8,len(x))
                        imgs = []
                        if plot_input:
            ...
            if plot_metrics:
                for metric_name, metric_val in zip(metrics.keys(),metrics.values()):
                    ...
                    plt.savefig(os.path.join(save_folder_G, metric_name + '_' + 'im_' + str(i) + '.png'), bbox_inches="tight")
                    if wandb_vis:
    ...
    if wandb_vis:
        table_psnr = wandb.Table(data=np.array(psnr_data), columns=['operator', 'image', 'Linear PSNR', 'Estimated PSNR'])
