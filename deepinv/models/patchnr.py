...
class PatchNR(nn.Module):
    ...
    """
    def __init__(self,normalizing_flow=None,pretrained_weights=None,patch_size=6,channels=1,num_layers=5,sub_net_size=256,device='cpu'):
        ...
        if normalizing_flow is None:
            ...
            self.normalizing_flow = Ff.ReversibleGraphNet(nodes, verbose=False).to(device)
        else:
    ...
    def forward(self,x):
        ...
        logpz=.5*torch.sum(latent_x.view(B,n_patches,-1)**2,-1)
