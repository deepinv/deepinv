...
class DEAL(Reconstructor):
    ...
    where :math:`K_{\theta,c}` are learned linear filters,
    ...
    :param str | None pretrained: checkpoint path, ``'download'``, ``'pretrained'``,
        or ``None``. If ``None``, no pretrained weights are loaded. If
        ``'download'`` or ``'pretrained'``, the official DEAL pretrained weights are
...
class LinearSplineDerivativeFunc(torch.autograd.Function):
    """
    Autograd function to only backpropagate through the B-splines that were
    used to calculate output = activation(input), for each element of the input.
    """
