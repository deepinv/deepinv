...
class EquivariantDenoiser(torch.nn.Module):
    ...
    def __init__(
        self, denoiser, rand_rot=False, mean_rot=False, rand_translations=False
    ):
        super().__init__()
    ...
    def forward(self, x, sigma, retain_grad=False):
        ...
        return denoise_rotate(
            ...
            x,
            sigma,
            rand_rot=self.rand_rot,
            mean_rot=self.mean_rot,
            rand_translations=self.rand_translations,
...
def denoise_rotate(
    ...
    if rand_rot:
        ...
        denoised = denoise_rotate_fn(
            denoiser, image, sigma, rot_idx=k, retain_grad=retain_grad
        )

    elif mean_rot:
        denoised = torch.zeros_like(image)
        for k in range(4):
            denoised = denoised + denoise_rotate_fn(
                denoiser, image, sigma, rot_idx=k, retain_grad=retain_grad
            )
        ...
        denoised = denoise_rotate_translate_fn(
            denoiser, image, sigma, rot_idx=k, translate_shift=(x_shift, y_shift)
        )
        ...
        denoised = denoise_rotate_fn(
            denoiser, image, sigma, rot_idx=0, retain_grad=retain_grad
        )
...
def denoise_rotate_translate_fn(
    denoiser, image, sigma, rot_idx=0, translate_shift=(0, 0)
):
    r"""
    Function applying the appropriate geometric transform, denoising and inverse transform to the input image.
    ...
    denoised = torch.rot90(denoised, k=-rot_idx, dims=[-2, -1])
    denoised = torch.roll(
        denoised, shifts=(-translate_shift[0], -translate_shift[1]), dims=(-2, -1)
    )
    return denoised
