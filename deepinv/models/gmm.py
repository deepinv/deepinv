...
class GaussianMixtureModel:
    ...
    def __init__(self,n_components,dimension,device='cpu'):
        ...
        self._cov=.1*torch.eye(dimension,device=device)[None,:,:].tile(n_components,1,1)
    ...
    def set_cov(self,cov):
        ...
        if self._covariance_regularization:
            self._cov_reg=self._cov+self._covariance_regularization*torch.eye(self.dimension,device=self._cov.device)[None,:,:].tile(self.n_components,1,1)
    ...
    def get_parameter_dict(self):
        ...
        """        
        parameters={'weights':self._weights.clone().cpu(),'mu':self.mu.cpu(),'cov':self._cov.clone().cpu()}
        return parameters
    ...
    def component_log_likelihoods(self,x,cov_regularization=False):
        ...
        component_log_likelihoods=-.5*logdet_cov[:,None]-.5*exponent-.5*self.dimension*torch.log(torch.tensor(2*torch.pi).to(x))
    ...
    def negative_log_likelihood(self,x):
        ...
        component_log_likelihoods=component_log_likelihoods+torch.log(self._weights[None,:])
    ...
    def classify(self,x,cov_regularization=False):
        ...
        :param bool cov_regularization: whether using regularized covariance matrices
        """
        component_log_likelihoods=self.component_log_likelihoods(x,cov_regularization=cov_regularization)
        component_log_likelihoods=component_log_likelihoods+torch.log(self._weights[None,:])
        val,ind=torch.max(component_log_likelihoods,1)
        return ind
        

    def fit(self,dataloader,max_iters=100,stopping_criterion=None,data_init=True,cov_regularization=1e-5,verbose=False):
        """
        ...
        for step in (progress_bar := tqdm(range(max_iters), disable=not verbose)):
            weights_new,mu_new,cov_new,objective_new=self._EM_step(dataloader,verbose)
            # stopping criterion
            self.set_weights=weights_new
            self.mu=mu_new
            cov_new_reg=cov_new+cov_regularization*torch.eye(self.dimension)[None,:,:].tile(self.n_components,1,1).to(cov_new)
            self.set_cov(cov_new_reg)
            if stopping_criterion:
                if objective-objective_new<stopping_criterion:
                    return
            objective=objective_new
            progress_bar.set_description("Step {}, Objective {:.4f}".format(step+1,objective.item()))
    ...
    def _EM_step(self,dataloader,verbose):
        ...
        for x,_ in tqdm(dataloader, disable=not verbose):
            ...
            C_new+=torch.bmm(beta_times_x.transpose(1,2),x[None,:,:].tile(self.n_components,1,1))
        mu_new=mu_new/weights_new[:,None]
        cov_new=C_new/weights_new[:,None,None]-torch.matmul(mu_new[:,:,None],mu_new[:,None,:])
