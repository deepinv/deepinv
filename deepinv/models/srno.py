...
class SRNO(nn.Module):
    ...
    def __init__(self, encoder_n_resblocks=16, encoder_n_feats=64, 
                 width=256, blocks=16):
        super().__init__()
        self.width = width
    ...
    def query_rgb(self, coord, cell):
        ...
        pos_lr = pos_lr.permute(2, 0, 1).unsqueeze(0).expand(feat.shape[0], 2, *feat.shape[-2:])
...
class SRNO_RDN(nn.Module):
    ...
    def __init__(self, encoder_G0=64, encoder_config='B', 
                 width=256, blocks=16):
        super().__init__()
        self.width = width
...
def load_model(checkpoint_path, device='cuda', encoder_type='auto'):
    ...
    print(f"Model config: encoder_out_dim={encoder_out_dim}, width={width}, blocks={blocks}")
