...
class DScCP(nn.Module):
    ...
    def __init__(self, K=20, F=64, device='cpu'):
        ...
        self.lip = torch.tensor(torch.ones(K),requires_grad=False,dtype=torch.float).cpu()
    ...
    def forward(self, x, sigma=0.03):
        ...
        for k in range(K):
            ...
            u = functional.hardtanh(u+tau/self.mu[k]*self.conv[k*2]((1+alphak)*x_curr-alphak*x_prev),min_val=-sigma**2,max_val=sigma**2)
            x_next = torch.clamp((self.mu[k]/(self.mu[k]+1))*x+(1/(1+self.mu[k]))*x_curr- (self.mu[k]/(self.mu[k]+1))* self.conv[k*2+1](u),min=0,max=1)
            x_prev= x_curr
            x_curr= x_next
