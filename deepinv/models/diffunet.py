...
class DiffUNet(Denoiser):
    ...
    This is the model with attention and timestep embeddings from :cite:`choi2021ilvr`;
    code is adapted from https://github.com/jychoi118/ilvr_adm.

    It is possible to choose the `standard model from :cite:`choi2021ilvr` with 128 hidden channels per layer (trained on FFHQ)
    and a larger model :cite:`dhariwal2021diffusion` with 256 hidden channels per layer (trained on ImageNet128).
