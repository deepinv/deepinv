...
class DiffusersDenoiserWrapper(Denoiser):
    ...
    def __init__(
        ...
        super().__init__()
        assert mode_id is not None, "Provide a diffusers model id. E.g., 'google/ddpm-cat-256'"
        
        try:
            from diffusers import DiffusionPipeline, DDPMScheduler
        except ImportError:
            raise ImportError("diffusers is not installed. Please install it via 'pip install diffusers'.")

        pipeline = DiffusionPipeline.from_pretrained(mode_id, torch_dtype=torch.float32)
        self.model = pipeline.unet
        self.scheduler = pipeline.scheduler

        assert isinstance(self.scheduler, DDPMScheduler), "Currently, only DDPMScheduler is supported."
        ...
        self.register_buffer('sqrt_one_minus_alpha_cumprod', (1.0 - ac).clamp(min=0).sqrt())  # sigma_t
        self.register_buffer('beta_cumprod', 1 - ac)

        # prediction_type: "epsilon", "v_prediction", or "sample"
        self.prediction_type = getattr(self.scheduler.config, "prediction_type", "epsilon")
    ...
    def forward(
        ...
        **kwargs,
        ...
        :param kwarg: additional keyword arguments to be passed to the model. For example, a `prompt` for text-conditioned models.
        ...
        if sigma is not None:
            if timestep is not None:
                warnings.warn("Both sigma and timestep are provided. Ignoring timestep and using sigma.")                
        ...
        pred = self.model(x, timestep).sample.to(dtype=dtype)
