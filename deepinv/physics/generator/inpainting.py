...
class GaussianSplittingMaskGenerator(BernoulliSplittingMaskGenerator):
    ...
    This mask is used for measurement splitting for MRI in :cite:`yaman2020self`.
    ...
    Handles both 2D mask (i.e. [C, H, W] from :cite:`yaman2020self` and 2D+time dynamic mask (i.e. [C, T, H, W] from :cite:`acar2021self` generation. Does not handle 1D data (e.g. of shape [C, M])
    ...
    def batch_step(self, input_mask: torch.Tensor = None) -> dict:
        ...
        Adapted from https://github.com/byaman14/SSDU/blob/main/masks/ssdu_masks.py from SSDU :cite:`yaman2020self`.
...
class Phase2PhaseSplittingMaskGenerator(BernoulliSplittingMaskGenerator):
    ...
    Used in :cite:`eldeniz2021phase2phase`.
...
class Artifact2ArtifactSplittingMaskGenerator(Phase2PhaseSplittingMaskGenerator):
    ...
    Artifact2Artifact was introduced in :cite:`liu2020rare`.
