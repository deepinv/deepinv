...
class TimeMixin:
    ...
    def flatten(x: torch.Tensor) -> torch.Tensor:
        ...
        :return torch.Tensor: output tensor of shape (B*T, C, H, W)
        """
        B, C, T, H, W = x.shape
    ...
    def unflatten(x: torch.Tensor, batch_size=1) -> torch.Tensor:
        ...
        :param torch.Tensor x: input tensor of shape (B*T, C, H, W)
    ...
    @staticmethod
    def wrap_flatten_C(f: Callable[[torch.Tensor], torch.Tensor]) -> torch.Tensor:
    ...
    def average(x: torch.Tensor, mask: torch.Tensor = None, dim: int = 2) -> torch.Tensor:
        ...
        :param torch.Tensor x: input tensor of shape (B,C,T,H,W) (e.g. time-varying k-space)
        :param torch.Tensor mask: mask showing where ``x`` is non-zero. If not provided, then calculated from ``x``.
        :param int dim: time dimension, defaults to 2 (i.e. shape B,C,T,H,W)
        :return torch.Tensor: flattened tensor with time dim removed of shape (B,C,H,W)
        """
        _x = x.sum(dim)
        out = zeros_like(_x)
    ...
    def repeat(x: torch.Tensor, target: torch.Tensor, dim: int = 2) -> torch.Tensor:
        ...
        :param torch.Tensor target: any tensor of desired shape (B,C,T,H,W)
        :param int dim: time dimension, defaults to 2 (i.e. shape B,C,T,H,W)
        :return torch.Tensor: tensor with new time dim of shape B,C,T,H,W
        """
