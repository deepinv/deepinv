...
class Physics(torch.nn.Module):  # parent class for forward models
    ...
    def set_ls_solver(self, solver, max_iter=None, tol=None):
        ...
        :param str solver: solver to use. If the physics are non-linear, the only available solver is `'gradient_descent'`.
            For linear operators, the options are `'CG'`, `'lsqr'`, `'BiCGStab'`, `'minres'` and `'lsmr'` (see :func:`deepinv.optim.linear.least_squares` for more details).
...
class LinearPhysics(Physics):
    ...
    :param str solver: least squares solver to use. Choose between `'CG'`, `'lsqr'`, `'BiCGStab'`, `'minres'` and `'lsmr'`. See :func:`deepinv.optim.linear.least_squares` for more details.
