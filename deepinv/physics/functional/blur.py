...
def gaussian_blur_nd(
    ...
    if isinstance(sigma, (int, float)):
        sigma = torch.tensor([[sigma] * dim] * batch_size, device=device, dtype=dtype)  # Shape: (batch_size, dim)
    ...
    # batch size of 1 with potentially anisotropic kernel
        ...
        sigma = torch.tensor([sigma] * batch_size, device=device, dtype=dtype)  # Shape: (batch_size, dim)
    ...
    if dim == 2:
        ...
        elif isinstance(angle, torch.Tensor) and (angle.dim() == 1 or angle.dim() == 2 and angle.shape[1] == 1) and angle.shape[0] == B:
