...
from utils import get_all_patches_and_positions
...
def unity_partition_function_1d(image_size, patch_size, overlap):
    ...
    return masks
    # diff = patch_size - overlap
    # return torch.stack([masks[i, i * diff: i * diff + patch_size + psf_size] for i in range(n_patch)], dim=0)
...
def multiply_tuple(a, constant):
    return tuple(int(a[i] * constant) for i in range(len(a)))
...
psf = get_psf_product_convolution2d_patches(
    h, w, position, overlap, image.shape[2:4])
...
#     (torch.arange(1, 256, 21), torch.arange(1, 256, 21)), dim=1)

# position = (25,25)
# psf_list = [get_psf_product_convolution2d_patches(
#     h, w, p, overlap, patches.shape[2:4]) for p in position]
