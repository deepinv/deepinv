...
class NonCartesianMRI(LinearPhysics):
    ...
    ----------
    E : object
        NUFFT backend with the following callables:

        - ``E.op(img) -> samples``:
          forward (non-Cartesian sampling).
          Input ``img`` may be [C, H, W, D] (per-coil image) or [H, W, D]
          (coil-combined image already multiplied by sensitivities), depending on
          your backend. Output is typically [C, M] or [M complex samples,
          where ``M`` is the number of k-space points.

        - ``E.adj_op(samples) -> img``:
    ...
    **kwargs :
    ...
    Notes
    -----
    - Linearity. DeepInv's optimization layers assume the physics ``A`` is
    ...
    - Coil sensitivities. This wrapper does not estimate or apply
      sensitivity maps. If you need SENSE/ESPIRiT, pre-apply them in your
      backend (e.g., pass coil-modulated images to ``op`` and perform the
      conjugate-weighted sum in your reconstruction algorithm).

    - Density compensation.If you use DCW (e.g., sqrt-dcf for a
      self-adjoint normal operator), incorporate it inside ``E.op``/``E.adj_op``
      consistently.

    - RI vs complex. DeepInv can work with real/imag stacked channels
      or with complex tensors. This wrapper passes through whatever you
      feed it. Decide on one convention for your project and stick to it.

    Examples
    --------
    Minimal wiring with L2 data fidelity and HQS in DeepInv:

    >>> physics = NonCartesianMRI(E)               # E provides .op / .adj_op
    >>> data_fid = deepinv.optim.data_fidelity.L2()
    >>> prior    = deepinv.optim.prior.TVPrior()
    ...
    ...     max_iter=15,
    ...     verbose=True,
    ...     params_algo={"stepsize": [1.0]*15},    # gamma schedule for prox on f
    ... )
    >>> x_rec, _ = model(y, physics)               # y produced by physics.A(x_gt)

    If your NUFFT backend returns NumPy arrays, add a thin adapter:

    >>> class TorchNUFFT:
    ...     def __init__(self, E, device):
    ...         self.E, self.device = E, device
    ...     def op(self, img_t):
    ...         y = self.E.op(img_t.detach().cpu().numpy())          # NumPy
    ...         return torch.as_tensor(y, device=self.device, dtype=torch.complex64)
    ...     def adj_op(self, y_t):
    ...         x = self.E.adj_op(y_t.detach().cpu().numpy())
    ...         return torch.as_tensor(x, device=self.device, dtype=torch.complex64)
