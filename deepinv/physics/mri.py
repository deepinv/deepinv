...
class NonCartesianMRI(LinearPhysics, MRIMixin):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if trajectory == "radial":
            self.samples_loc = mrinufft.initialize_2D_radial(
                Nc=num_shots, Ns=num_samples_per_shot
            )
        elif trajectory == "spiral":
            self.samples_loc = mrinufft.initialize_2D_spiral(
                Nc=num_shots, Ns=num_samples_per_shot, nb_revolutions=1
            )

        self.backend = backend
        self.nufft = mrinufft.get_operator(self.backend)(
            self.samples_loc, shape=img_size, n_coils=1, n_batchs=1
        )  # we do batching ourselves
        ...
        if self.density_mode is None:
            ...
            self.density = (
                torch.from_numpy(mrinufft.density.voronoi(self.samples_loc))
                .to(device)
                .view(1, 1, -1)
            )  # 111S
        else:
        ...
        if normalize:
            self.operator_norm = self.compute_norm(
                torch.randn(1, 2, *img_size[-2:], device=device), squared=False
            )
        ...
        for _x in x.double():
            ...
            if self.backend == "finufft":
                out = self.from_torch_complex(
                    self.nufft.op(self.to_torch_complex(_x)).unsqueeze(0)  # 1HW  # 1S
                )  # 12S

            elif self.backend == "cufinufft":
                out = self.from_torch_complex(
                    self.nufft.op(
                        self.to_torch_complex(
                            _x
                        )  # .unsqueeze(1) # 11HW # TODO is unsqueeze here needed?
                    )  # 11S
                )  # 121S # should be 12S
    ...
    def A_adjoint(self, y: torch.Tensor) -> torch.Tensor:  # B2S -> B2HW
        ...
        for _y in y_in:
            ...
            if self.backend == "finufft":
                out = self.from_torch_complex(
                    self.nufft.adj_op(self.to_torch_complex(_y)).unsqueeze(  # 1S
                        0
                    )  # 1HW
                )  # 12HW
            elif self.backend == "cufinufft":
                out = self.from_torch_complex(
                    self.nufft.adj_op(self.to_torch_complex(_y))  # 1S  # 1HW
                )  # 12HW
