...
class PolyOrderMaskGenerator(BaseMaskGenerator):
    ...
    def get_pdf(self) -> torch.Tensor:
        ...
        center_start = self.W // 2 - self.n_center // 2
        center_end = self.W // 2 + math.ceil(self.n_center / 2)
        pdf[center_start:center_end] = 1
        ...
        while iterations < 100:
            ...
            scaled_pdf[center_start:center_end] = 1  # Ensure center is fully sampled
    ...
    def sample_mask(self, mask: torch.Tensor) -> torch.Tensor:
        ...
        pdf = self.variable_density_pdf()  # Correct function call

        for b in range(mask.shape[0]):  # Iterate over batch
            for t in range(mask.shape[2]):  # Iterate over time steps
                # Broadcast probability mask to match (H, W)
                mask[b, :, t, :, :] = torch.bernoulli(pdf.unsqueeze(0).expand(mask.shape[3], 1))
