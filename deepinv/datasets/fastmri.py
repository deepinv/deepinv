    ...
    **Raw data file structure:** ::
    ...
    def __init__(
        ...
        if not os.path.isdir(root):
            raise ValueError(f"The `root` folder doesn't exist. Please set `root` properly. Current value `{root}`.")
        # check that root folder contains only hdf5 files
        if not all([file.endswith(".h5") for file in os.listdir(root)]):
            raise ValueError(f"The `root` folder doesn't contain only hdf5 files. Please set `root` properly. Current value `{root}`.")
        ...
        elif volume_sample_rate < 1.0:  # sample by volume / randomly keep a portion of mri scans
