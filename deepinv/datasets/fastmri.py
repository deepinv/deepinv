...
class SimpleFastMRISliceDataset(torch.utils.data.Dataset):
    ...
    Generate the saved subset using the ``from_FastMRISliceDataset`` method.

    We provide a pregenerated saved subset for the FastMRI knees. You can load this using :class:`deepinv.utils.demo.load_dataset`
    and specifying the dataset name as ``fastmri_knee_singlecoil`` which will download this subset from our HuggingFace repo.
...
class FastMRISliceDataset(torch.utils.data.Dataset):
    ...
    and ``target`` are the root-sum-square reconstructions of shape ``(2, H, W)``.
    ...
    **Raw data file structure:** ::
    ...
    .. note::
        ...
        Key modifications include cleaning up some messy code and the following:
        - Use torch.randperm instead of random shuffle
        - Option to convert to SimpleFastMRISliceDataset
        - Return torch tensors
        - Add slice_index parameter
        - Remove redundant challenge parameter and sample_rate (this can be easily achieved using torch.utils.data.Subset)
    ...
    def __getitem__(self, idx: int) -> Tuple[Any, Any]:
        ...
        with h5py.File(fname, "r") as hf:
            ...
            if not self.test:
                ...
                target = torch.stack([target, torch.zeros_like(target)])  # (2, H, W)

        return kspace if self.test else (target, kspace)

    def save_simple_dataset(
        self, dataset_path: str, pad_to_size: Tuple[int] = (320, 320)
        ...
        if pad_to_size is not None:
            self.transform_target = Compose(
                [transform_target]
                if transform_target is not None
                else [] + [Rescale(), CenterCrop(pad_to_size)]
            )
        ...
        return SimpleFastMRISliceDataset(dataset_path, train=True, transform=None)
