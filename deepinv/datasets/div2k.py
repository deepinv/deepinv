...
class DIV2K(torch.utils.data.Dataset):
    ...
    def __init__(self, root: str, mode: str='train', download: bool=False, transform=None) -> None:
        ...
        # otherwise we try to download the whole dataset
            ...
            for filename, url in self.zipfile_urls.items():
                ...
                download_zipfile(url=self.zipfile_urls[filename], save_path=os.path.join(self.root, filename))
    ...
    def _check_dataset_exists(self):
        ...
        if not data_dir_exist:
            return False
        return all(
                calculate_md5_for_folder(os.path.join(self.root, split)) == checksum for split, checksum in self.checksums.items()
            )
